    ...
    public class HuntController
    {
        ...
        public void Command(string args, Action<string> reply)
        {
            ...
            if (a == "why")
            {
                ...
                foreach (var l in lines) _ctx.Log("HUNT why:   " + l);
                reply(lines.Count == 0 ? "No NPCs near me." : $"{lines.Count} NPCs near me (full list in my log): " + string.Join(" | ", lines.Take(8)));
                return;
            }
            if (a.Length > 0)
            {
                ...
                { reply("Usage: hunt [radius 5-100] | hunt off | hunt status | hunt why"); return; }
                Radius = r;
            }
        ...
        public SimpleChar Tick(LocalPlayer me, PlayerChar owner, double dt)
        {
            ...
            // The pet walks from where it is, so the next mob is the one nearest the PET: ranking by distance
            // from the bot sent it running past mobs on its way back toward the bot.
            var petDyn = me.Pets.Where(p => pets.Contains(p.Identity)).ToList();
            Vector3 from = petDyn.Count > 0 ? petDyn[0].Transform.Position : centre;
