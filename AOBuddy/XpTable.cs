...
namespace AOBuddy
{
    ...
    /// character's TOTAL XP (observed live, 2026-09-26 - it is NOT the progress
    /// inside the current level), so the way through the current level is
    /// (XP - Sum(table[1..level-1])) / table[level].
    /// </summary>
    internal static class XpTable
    {
        ...
        /// simply XP of 1450, the way to level 2. -1 when the level is outside the table (150+).
        /// </summary>
